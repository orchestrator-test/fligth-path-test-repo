...
echo 'building and pushing image flightpathtestrepo-buildstage'
cd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/flightpathtestrepo-buildstage .
cd -

echo 'building and pushing image flightpathtestrepo'
cd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/flightpathtestrepo .
cd -
