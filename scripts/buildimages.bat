...
echo "building image flightpathtestrepo-buildstage"
pushd source
%CONTAINER_RUNTIME% build -f Dockerfile.buildstage -t flightpathtestrepo-buildstage .
popd

echo "building image flightpathtestrepo"
pushd source
%CONTAINER_RUNTIME% build -f Dockerfile -t flightpathtestrepo .
popd
